...
public class EyeTrackingExample : MonoBehaviour
{
    ...
    private static readonly string[] ColumnNames = { "Trial", "Time",
        "Frame", "CaptureTime", "LogTime", "HMDPosition", "HMDRotation", "GlobalCameraPosition", "CameraForward",
        "GazeStatus", "CombinedGazeForward", "CombinedGazePosition", "CombinedGazeForwardGlobal", "CombinedGazePositionGlobal", "GazeHitObject","GazeHitPosition", "InterPupillaryDistanceInMM", 
    ...
    {
        string[] logData = new string[31];

        // * Trial *
        logData[0] = ColorGlobal.trial.ToString();
        ...
        logData[18] = leftInvalid ? "" : data.left.forward.ToString("F3");
        logData[19] = leftInvalid ? "" : data.left.origin.ToString("F3");
        logData[20] = leftInvalid ? "" : eyeMeasurements.leftPupilIrisDiameterRatio.ToString("F3");
        logData[21] = leftInvalid ? "" : eyeMeasurements.leftPupilDiameterInMM.ToString("F3");
        logData[22] = leftInvalid ? "" : eyeMeasurements.leftIrisDiameterInMM.ToString("F3");

        // Right eye
        bool rightInvalid = data.rightStatus == VarjoEyeTracking.GazeEyeStatus.Invalid;
        logData[23] = rightInvalid ? InvalidString : ValidString;
        logData[24] = rightInvalid ? "" : data.right.forward.ToString("F3");
        logData[25] = rightInvalid ? "" : data.right.origin.ToString("F3");
        logData[26] = rightInvalid ? "" : eyeMeasurements.rightPupilIrisDiameterRatio.ToString("F3");
        logData[27] = rightInvalid ? "" : eyeMeasurements.rightPupilDiameterInMM.ToString("F3");
        logData[28] = rightInvalid ? "" : eyeMeasurements.rightIrisDiameterInMM.ToString("F3");

        // Focus
        logData[29] = invalid ? "" : data.focusDistance.ToString();
        logData[30] = invalid ? "" : data.focusStability.ToString();
